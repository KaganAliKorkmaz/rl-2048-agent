...
import torch
from src.Policy_Gradient import train_reinforce_with_baseline, evaluate_policy_advanced
...
if __name__ == "__main__":
    if torch.cuda.is_available():
        ...
        device = "cpu"
    print("=" * 80)
    print("Experiment 6: REINFORCE (Monte Carlo Policy Gradient) with Action Masking")
    print("=" * 80)
    ...
    policy_net, value_net, training_scores, eval_results = train_reinforce_with_baseline(
        ...
        max_grad_norm=0.5,         
        checkpoint_dir=os.path.join(os.path.dirname(__file__), '..', 'checkpoints', 'reinforce_advanced'),
        curriculum_stages=[
            (0, 3000, 128),        
            (3000, 7000, 256),      
            (7000, None, None),    
    ...
    if len(training_scores) > 0:
        ...
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(results_dir, 'reinforce_advanced_training_curve.png'), dpi=150, bbox_inches='tight')
    ...
    if len(eval_results) > 0:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(results_dir, 'reinforce_advanced_evaluation_curve.png'), dpi=150, bbox_inches='tight')
